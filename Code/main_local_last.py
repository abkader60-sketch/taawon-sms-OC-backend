...
Changes in this revision (30 Apr 2026, v0.6):
  + Site_Access_ID gets two new optional columns:
      date_joined   (DATE)
      reports_to    (VARCHAR(100))
    Used by the new 'brief' form variant for Group-1 organizations
    (Client / PMC / Main / LDC). Both columns are nullable so existing rows
    are unaffected, and they are also accepted on the comprehensive form
    submission - they're just optional fields, not a separate code path.
  + The submit endpoint accepts both new fields and writes them through.
  + Excel export adds the two columns to its output.

Carried forward from v0.5:
  Submitter_Organizations, External_Submitters, System_Settings,
  workflow_mode toggle, edit_application, /applications/export,
  ~6 admin endpoints for external submitters + settings, bcrypt password
  hashing, credential email automation.
...
#  App setup
# ============================================================
app = FastAPI(title="Ta'awon P3 SMS API", version="0.6.0")
...
-- v0.6: brief-form variants don't have all the previously-NOT-NULL fields.
--       Relax those columns so the brief form can submit successfully.
...
MIGRATION_SQL = """
UPDATE App_Users SET role_id = 5 WHERE user_id = 2 AND role_id = 2;
...
class ApplicationFieldsUpdate(BaseModel):
    ...
    date_joined:      Optional[str] = None   # YYYY-MM-DD or empty string to clear
...
def parse_date_or_none(s: Optional[str]) -> Optional[date]:
    """Convert an empty string or a YYYY-MM-DD string to a date. Returns None for falsy."""
    if not s:
        return None
    try:
...
async def submit_application(
    # Always-present fields (full + brief variants both supply these)
    full_name: str  = Form(...),
    organization: str  = Form(...),
    email: Optional[str] = Form(None),
    submitted_by_user_id: int = Form(...),
    form_variant: str = Form("full"),  # 'full' or 'brief'
    # Conditionally-required fields. Brief form may omit some.
    ...
    date_joined: Optional[str] = Form(None),       # YYYY-MM-DD
...
async def edit_application_fields(application_id: int, payload: ApplicationFieldsUpdate,
    ...
    try:
        ...
        # date_joined needs date parsing
